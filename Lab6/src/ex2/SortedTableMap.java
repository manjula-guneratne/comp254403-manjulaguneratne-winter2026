...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
   ...
   * Uses binary search via findIndex for O(log n) performance.
   * Resolves the ambiguity where get(k) returning null could mean
   * either "key not found" or "key maps to a null value".
   *
   * @param key  the key to search for
   * @return true if an entry with the key exists, false otherwise
   */
  public boolean containKey(K key) {
    int j = findIndex(key);                          // binary search for key position
    if (j == size()) return false;                   // key would be beyond last entry
    return compare(key, table.get(j)) == 0;          // true only on exact match
  }
